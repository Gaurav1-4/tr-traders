...
const Home = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg overflow-x-hidden">
      ...
      {/* ===== SIGNATURE HERO REEL - UNIVERSAL GRID ===== */}
            ...
            <div key={i} className={`relative h-full overflow-hidden group bg-black transition-all duration-1000 ${heroVideos.length > 3 ? 'flex-shrink-0 w-[85vw] md:w-[33.33vw] snap-center border-r border-white/10' : 'w-full border-r border-white/5 last:border-r-0'}`}>
               {videoErrors[i] ? (
                 <div className="absolute inset-0 bg-[#0D1B38] flex items-center justify-center p-8 text-center">
                    <p className="text-white/20 text-[8px] uppercase font-black tracking-[0.5em]">Curating...</p>
                 </div>
               ) : (
                 <video 
                   preload="auto"
               ...
               <div className="absolute inset-x-0 bottom-0 py-16 bg-gradient-to-t from-black via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-1000 select-none hidden md:flex flex-col items-center">
                  <span className="text-white font-black text-[9px] uppercase tracking-[0.6em]">Heritage Exhibition {i+1}</span>
               </div>
            </div>
          ))}
        </div>
      ...
      <section className="py-24 md:py-48 max-w-[1800px] mx-auto px-10 lg:px-24 w-full flex-grow bg-bg">
        <div className="flex flex-col items-center justify-center mb-36 text-center animate-fade-in px-4">
          <h1 className="text-7xl md:text-[11rem] font-serif font-light text-text tracking-tighter mb-12 italic leading-none opacity-90">Heritage</h1>
          <div className="w-56 h-0.5 bg-[#0D1B38]/10 mb-12"></div>
          <p className="max-w-2xl text-[#0D1B38]/40 text-[10px] md:text-[13px] uppercase tracking-[0.6em] font-black leading-loose">
             Authenticity in design, <br/> crafted for the modern luxury visionary.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-10 md:gap-x-20 gap-y-28 md:gap-y-64">
          {loading ? (
            ...
            <div className="col-span-full py-80 text-center text-muted border border-dashed border-[#0D1B38]/5 rounded-[7rem] bg-[#0D1B38]/[0.01]">
        ...
        <div className="mt-56 flex justify-center pb-24 px-4">
          <Link to="/catalog" className="w-full md:w-auto text-center group relative border-[3px] border-[#0D1B38] text-[#0D1B38] md:px-40 py-16 hover:bg-[#0D1B38] hover:text-white transition-all duration-1000 uppercase tracking-[0.8em] text-[15px] font-black shadow-[0_50px_130px_-30px_rgba(13,27,56,0.4)] active:scale-95 hover:-translate-y-3 block">
